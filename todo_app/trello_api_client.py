...
class TrelloAPIClient():
    ...
    def get_items_on_a_board(self, board_id):
        ...
        for item in response:            
            cards.append(Item(item['id'], item['name'], item['desc'], self.format_datetime_string_as_date(item['due']), item['idList'], item['idBoard'], self.get_list_name(item['idList'])))
        return cards
    ...
    def create_item(self, list_id, title, description, due_date):
        ...
        card = Item(response['id'], response['name'], response['desc'], self.format_datetime_string_as_date(response['due']), response['idList'], response['idBoard'], self.get_list_name(response['idList']))
        return card
    ...
    def format_datetime_string_as_date(self, date_time):
        if date_time != None:
